...
5. **Traceable Page-Bound Chunking (`CorpusA_Chunks/`)**: Multi-page parent documents are isolated into separate subdirectories with parent IDs baked into every chunk name (`A-MPEDA-004-C01.txt`), preserving physical PDF page boundaries (`--- PAGE N ---`) and inheriting exact empirical parent DQA grades inside `chunk_manifest.csv`.
