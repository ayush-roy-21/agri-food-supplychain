...
5. **Traceable Page-Bound & Overlap-Aware Chunking (`CorpusA_Chunks/` & `CorpusB_Chunks/`)**: Multi-page and extended parent documents across both Corpus A and Corpus B candidate pools (`52 candidate documents`) are isolated into separate subdirectories with parent IDs baked into every chunk name (`A-MPEDA-004-C01.txt`, `B-GD-071-C01.txt`). Enforces exact precision size thresholds (`~250 words target`, `400 words hard cap`, `90 words floor`, `~30 words overlap`), preserving physical PDF page boundaries (`--- PAGE N ---`) and inheriting empirical parent DQA grades across **1,772 verified chunks** indexed inside `chunk_manifest.csv` (optimized for `BERTopic` clustering with `min_cluster_size=8..10`).
6. **Traceable Locus Tags & Streamlined Pipeline**: Concrete linking of every file across both corpora to specific statutory mechanisms, managed through a streamlined inventory of exactly **8 modular production scripts** (`src/data-processing/`).
