...
1. **`chunk_documents.py`**: Standardized page-boundary (`--- PAGE N ---`) and overlap-aware chunking engine enforcing precision embedding size thresholds (`~250 words target`, `400 words hard cap`, `90 words floor`, `~30 words overlap`). Preserves single-chunk candidate documents above word thresholds (`>= 600 words` Corpus A, `>= 1,400 words` / `B-GD-071` Corpus B) and logs exact parent DQA inheritance across **1,772 verified chunks** (`chunk_manifest.csv`).
