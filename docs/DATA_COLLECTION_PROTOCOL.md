# The Barrier Horizon: Sustainable Supplier Hurdles Facing Indian Agri-Food 
...
1. **`chunk_documents.py`**: Standardized page-boundary (`--- PAGE N ---`), overlap-aware, and sub-linearly down-weighted chunking engine enforcing precision embedding size thresholds (`~250 words target`, `400 words hard cap`, `90 words floor`, `~30 words overlap`). Preserves single-chunk candidate documents above word thresholds (`>= 600 words` Corpus A, `>= 1,400 words` / `B-GD-071` Corpus B). Applies the **Ceiling Square-Root Rule & Stratified Sampling (`DEC-2026-025`)** (`numpy.linspace` to sample $N_{sampled} = \lceil \sqrt{N} \rceil$ chunks) to prevent long documents from skewing topic modeling, reducing 1,772 raw chunks to exactly **276 verified balanced chunks** (`chunk_manifest.csv`) with exact parent DQA inheritance and distinct parent tracking.
