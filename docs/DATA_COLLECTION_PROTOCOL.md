...
## Data Collection & Quality Assessment Protocol (v1.1.0)
...
All one-off, file-specific fix scripts and temporary simulation tools have been purged from the repository to maintain clean modular architecture. The production pipeline includes the following key standardized reusable modules (out of 36 total scripts):
...
- **Topic Modeling Methodology (`Chapter 2`)**: Explains how clean DQA-passed documents (`259 records`) and down-weighted chunks (`383 chunks`) transition into the 345-unit vector space (after noise gating) (`intfloat/e5-base-v2`) and undergo class-based TF-IDF enrichment (`topics_per_class`).
- **Reproducibility Guarantee (`Chapter 4`)**: All candidate thresholds and data extraction rules correspond exactly to release tag **`v1.1.0`**, verified automatically by **`[verify_pipeline_integrity.py](file:///e:/Summer%20Internship%2726/agri-food-project/src/data-processing/verify_pipeline_integrity.py)`**.
